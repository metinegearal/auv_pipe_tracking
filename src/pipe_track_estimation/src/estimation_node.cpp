...
void EstimationNode::mag_callback(const sensor_msgs::msg::MagneticField::SharedPtr msg) {
    ...
    current_yaw_ = std::atan2(msg->magnetic_field.x, msg->magnetic_field.y) - M_PI / 2.0; // Adjust for coordinate frame if necessary
...
void EstimationNode::dvl_callback(const geometry_msgs::msg::TwistStamped::SharedPtr msg) {
    Eigen::Vector3d linear_vel(msg->twist.linear.x,
                               msg->twist.linear.y,
                               msg->twist.linear.z);
    ...
    const double timestamp = 1.0 / 30.0; // Placeholder for timestamp; in a real system, use msg->header.stamp
    ...
    RCLCPP_INFO(this->get_logger(), "Fused Pose: [%.3f, %.3f, %.3f]", 
                current_pose.translation().x()+43.203, 
                current_pose.translation().y()-64.463, 
                current_pose.translation().z()-24.178);
}
