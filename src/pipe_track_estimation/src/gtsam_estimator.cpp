...
void GtsamEstimator::initialize(
  ...
  const gtsam::imuBias::ConstantBias & initial_bias)
{
  ...
  imu_preintegrated_ =
    std::make_shared<gtsam::PreintegratedImuMeasurements>(preintegration_params, initial_bias);
...
void GtsamEstimator::add_imu_measurement(
...
void GtsamEstimator::add_vo_measurement(const gtsam::Pose3 & vo_pose, double /*timestamp*/)
{
  ...
  gtsam::ImuFactor imu_factor(
    X(state_index_ - 1), V(state_index_ - 1), X(state_index_), V(state_index_), B(state_index_ - 1),
    *imu_preintegrated_);
  ...
  gtsam::NavState prop_state =
    imu_preintegrated_->predict(gtsam::NavState(current_pose_, current_velocity_), current_bias_);
...
void GtsamEstimator::add_dvl_measurement(
  const Eigen::Vector3d & linear_velocity_body, double /*timestamp*/, double absolute_yaw)
{
  std::lock_guard<std::mutex> lock(graph_mutex_);
  ...
  double current_roll = current_pose_.rotation().roll();
  double current_pitch = current_pose_.rotation().pitch();

  // Anchor the constraint to the CURRENT position, not (0,0,0)
  gtsam::Pose3 yaw_constraint(
    gtsam::Rot3::Ypr(absolute_yaw, current_pitch, current_roll), current_pose_.translation());
