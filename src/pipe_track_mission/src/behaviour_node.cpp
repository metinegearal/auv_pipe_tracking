...
public:
  BehaviourNode() : Node("behaviour_node")
  {
    ...
    ctx_->stop_pub =
      this->create_publisher<std_msgs::msg::Bool>("/movement/finished_execution", 10);

    // Register BT Nodes
    BT::BehaviorTreeFactory factory;
    factory.registerBuilder<IsExplorationEmpty>(
      "IsExplorationEmpty", [this](const std::string & name, const BT::NodeConfig &) {
        return std::make_unique<IsExplorationEmpty>(name, ctx_);
      });

    factory.registerBuilder<IsConstantlyTurningBack>(
      "IsConstantlyTurningBack", [this](const std::string & name, const BT::NodeConfig &) {
        return std::make_unique<IsConstantlyTurningBack>(name, ctx_);
      });

    factory.registerBuilder<StopAUV>(
      "StopAUV", [this](const std::string & name, const BT::NodeConfig & config) {
        return std::make_unique<StopAUV>(name, config, ctx_);
      });
