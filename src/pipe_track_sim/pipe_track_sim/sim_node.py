...
class HoloOceanROS2Bridge(Node):
    ...
    def __init__(self, holoocean_make_fn, step_limit=None):
        """Initialize the HoloOcean ROS2 bridge."""
        ...
        self.command = [10, 10, 10, 10, 0, 0, 0, 0]
    ...
    def _publish_camera(self, np_img):
        """Publish a camera image and CameraInfo message."""
        try:
            if np_img is None:
                return
    ...
    def _publish_camera_down(self, np_img):
        """Publish the downward-facing camera image."""
        try:
            if np_img is None:
                return
            if np_img.dtype != np.uint8:
    ...
    def _publish_depth(self, depth_data):
        """Publish a scalar or image-form depth measurement.
            ...
            if depth_np.ndim == 2:
                ...
                try:
                    ...
                    self.get_logger().warn(
                        'Unsupported depth shape: %s' % (str(depth_np.shape),)
                    )
        except Exception as e:
    ...
    def _publish_imu(self, imu_data):
        """Publish IMU data from dictionaries or arrays."""
        try:
            if imu_data is None:
                return
            msg = Imu()
    ...
    def _publish_dvl(self, dvl_data):
        """Publish DVL velocity and distance data."""
        try:
            if dvl_data is None:
                return
            msg = TwistStamped()
    ...
    def _publish_range_finder(self, range_data):
        """Publish a scalar range finder value."""
        try:
    ...
    def _publish_location(self, loc_data):
        """Publish an Odometry message from location data."""
        try:
            if loc_data is None:
                return
            msg = Odometry()
    ...
    def _run_holoocean_loop(self):
        """Run the HoloOcean loop until the environment exits or the limit is reached."""
            ...
            try:
                while self._running:
                    if self.step_limit is not None and steps >= self.step_limit:
                        self.get_logger().info(
                            'Reached step limit (%d).' % self.step_limit
                        )
                        break
                    ...
                    try:
                        try:
                            ...
                            self.get_logger().debug(
                                'camera publish failed: %s' % traceback.format_exc()
                            )
                    ...
                    try:
                        range_finder = state.get(
                            'RangeFinder', None) if isinstance(
                            state, dict) else getattr(state, 'RangeFinder', None)
                        self._publish_range_finder(range_finder)
                    except Exception:
                        self.get_logger().debug(
                            'range finder publish failed: %s' % traceback.format_exc()
                        )
                    ...
                    try:
                        ...
                        self.get_logger().debug(
                            'depth failed: %s' % traceback.format_exc()
                        )
                    ...
                    try:
                        ...
                        self.get_logger().debug(
                            'mag failed: %s' % traceback.format_exc()
                        )
                    ...
                    try:
                        ...
                        self.get_logger().debug(
                            'location failed: %s' % traceback.format_exc()
                        )
                ...
                self.get_logger().error(
                    'Exception inside HoloOcean main loop: %s' % str(e)
                )
                self.get_logger().error(traceback.format_exc())
            finally:
                self.get_logger().info(
                    'HoloOcean context exited after %d steps.' % steps
                )
