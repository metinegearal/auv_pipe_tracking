    ...
    const cv::Mat& mask_image, int look_ahead_pixels, double angle_weight, bool use_skeleton) 
{
    ...
    if (use_skeleton) {
        ...
        if (!contours.empty()) {
            ...
            cv::drawContours(clean_skeleton, std::vector<std::vector<cv::Point>>{*largest_contour}, -1, cv::Scalar(255), 1);
    ...
    if (points.empty()) {
        return {0.0, cv::Point(center_x, center_y), clean_skeleton, cv::Mat::zeros(clean_skeleton.size(), CV_64F)};
    }
