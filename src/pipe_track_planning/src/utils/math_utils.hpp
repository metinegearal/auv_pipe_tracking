...
inline double calculate_angle_from_center_to_pixel(double px, double fov = 90.0, double width = 288.0) {
    double fx = (width / 2.0) / std::tan((fov / 2.0) * M_PI / 180.0);
    double cx = width / 2.0;
    return std::atan((px - cx) / fx);
}

inline std::vector<double> point_from_depth(const std::vector<double>& center, double yaw, double depth, bool downcamera = false) {
