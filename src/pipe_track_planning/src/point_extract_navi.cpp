...
public:
    PointExtractNavi() : Node("point_extract_navi_node") {
        ...
        position_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "holocean/odom", 10, std::bind(&PointExtractNavi::position_callback, this, std::placeholders::_1));
        height_sub_ = this->create_subscription<std_msgs::msg::Float32>(
            "holocean/depth/distance", 10, std::bind(&PointExtractNavi::depth_callback, this, std::placeholders::_1));
...
private:
    ...
    void cam_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
        ...
        std::vector<float> wp_data = {
            static_cast<float>(target_point.x), 
            static_cast<float>(target_point.y), 
            -28.0f 
        };
        wp_msg.data = wp_data;
