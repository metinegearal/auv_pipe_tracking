...
std::pair<cv::Point2f, int> Explore2D::camera_to_world_coordinates(double px, double py, double depth, double fov, cv::Size img_size) {
    auto local_pt = math_utils::point_from_depth({px, py}, yaw_, depth, true);
    int status = 1;
    
    if (px < img_size.width * (1.0 - SAFE_FRAME_) || px >= img_size.width * SAFE_FRAME_ ||
...
void Explore2D::calculate_ideal_path() {
    ...
    for (const auto& wp : actual_path_) {
        ...
        if (!local_cells.empty()) {
            ...
            if (!raw_ideal_path.empty()) raw_ideal_path.push_back(raw_ideal_path.back());
            else raw_ideal_path.push_back(wp);
        }
    }
...
void Explore2D::add_points(const std::vector<MapPoint>& points, cv::Size img_size, double depth, const cv::Point2f& position, double yaw) {
    yaw_ = yaw;
    world_point_ = position;
    
    std::cout << "Adding points with position=[" << position.x << ", " << position.y << "] yaw=" << yaw << " depth=" << depth << "\n";
    ...
    for (const auto& p : points) {
        ...
        double obj = p.obj;
        if (status == 2) {
            if (obj == 1.0) obj = status;
            else continue;
        }
        point_map_scanned_.push_back({(int)(world_pt.x / MAP_RESOLUTION), (int)(world_pt.y / MAP_RESOLUTION), obj});
    }
...
cv::Mat Explore2D::get_map() {
    ...
    for (const auto& [key, val] : final_map_) {
        min_x = std::min(min_x, key.first); max_x = std::max(max_x, key.first);
        min_y = std::min(min_y, key.second); max_y = std::max(max_y, key.second);
    }
    ...
    for (const auto& [key, status] : final_map_) {
        ...
        if (status == 2.0) color = cv::Vec3b(255, 255, 0);
        else if (status >= 0.1) color = cv::Vec3b(255, 0, 0);
