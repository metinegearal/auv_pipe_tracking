...
public:
          ...
          pid_yaw_(0.6, 0.3, 0.1, 0.0, 0.2) 
    {
        ...
        // Subscribers
        waypoint_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>("/trajectory/waypoint", 10, std::bind(&WaypointControlNode::waypoint_callback, this, _1));
        position_sub_ = this->create_subscription<nav_msgs::msg::Odometry>("holocean/odom", 10, std::bind(&WaypointControlNode::position_callback, this, _1));
        distance_sub_ = this->create_subscription<std_msgs::msg::Float32>("holocean/depth/distance", 10, std::bind(&WaypointControlNode::dist_callback, this, _1));
        depth_sub_ = this->create_subscription<std_msgs::msg::Float32>("holocean/depth/scalar", 10, std::bind(&WaypointControlNode::depth_callback, this, _1));
        sub_mag_ = this->create_subscription<sensor_msgs::msg::MagneticField>("holocean/mag", 10, std::bind(&WaypointControlNode::magnetometer_callback, this, _1));
        obstacle_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>("object/obstacle", 10, std::bind(&WaypointControlNode::obstacle_callback, this, _1));
        angle_sub_ = this->create_subscription<std_msgs::msg::Float32>("/aruco/angle", 10, std::bind(&WaypointControlNode::angle_callback, this, _1));
        mode_sub_ = this->create_subscription<std_msgs::msg::String>("/movement/mode", 10, std::bind(&WaypointControlNode::mode_callback, this, _1));
        finished_execution_sub_ = this->create_subscription<std_msgs::msg::Bool>("/movement/finished_execution", 10, std::bind(&WaypointControlNode::finished_execution_callback, this, _1));
        dvl_reset_sub_ = this->create_subscription<std_msgs::msg::Bool>("/movement/dvl_reset", 10, std::bind(&WaypointControlNode::dvl_reset_callback, this, _1));
...
private:
    ...
    // --- Original Callbacks ---
    void finished_execution_callback(const std_msgs::msg::Bool::SharedPtr msg) { finished_execution_ = msg->data; }
    ...
    void position_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        if (dvl_active_) current_position_ = {pos.x, pos.y, pos.z};
        else current_position_ = {0.0, 0.0, pos.z};
    ...
    void dist_callback(const std_msgs::msg::Float32::SharedPtr msg) { current_dist_seabed_ = msg->data; }
    ...
    void move_towards_waypoint(std::vector<double> target) {
        ...
        RCLCPP_INFO(this->get_logger(), "Distance to waypoint: %.2f, Target: (%.2f, %.2f, %.2f), Current: (%.2f, %.2f, %.2f)", distance, target[0], target[1], target[2], current_position_[0], current_position_[1], current_position_[2]);
        ...
        double yaw_error = hardcoded_angle_.has_value() ? hardcoded_angle_.value() 
        ...
        if (std::abs(yaw_error) > yaw_deadband && (mode_ == "yaw_centerize" || mode_ == "full_centerize")) {
    ...
    std::vector<char> determine_keys(const std::vector<double>& dir) {
        ...
        if (obstacles_.size() > 4 && obstacles_[4] == 0.0) {
            if (dir[0] > 0.1) keys.push_back('w');
            else if (dir[0] < -0.1) keys.push_back('s');
            if (dir[1] > 0.1) keys.push_back('d');
            else if (dir[1] < -0.1) keys.push_back('a');
        } else {
            if (dir[0] > 0.1) keys.push_back('d');
            else if (dir[0] < -0.1) keys.push_back('a');
        }

        if (dir[2] > 0.1) keys.push_back('i');
        else if (dir[2] < -0.1) keys.push_back('k');
        return keys;
    }

    std::vector<double> parse_keys(const std::vector<char>& keys, double val, std::vector<double> cmd) {
        auto has_key = [&](char k) { return std::find(keys.begin(), keys.end(), k) != keys.end(); };
        
        if (has_key('i')) { for(int i=0; i<4; i++) cmd[i] += val; }
        if (has_key('k')) { for(int i=0; i<4; i++) cmd[i] -= val; }
        if (has_key('j')) { cmd[4] += val; cmd[7] += val; cmd[5] -= val; cmd[6] -= val; }
        if (has_key('l')) { cmd[4] -= val; cmd[7] -= val; cmd[5] += val; cmd[6] += val; }
        if (has_key('w')) { for(int i=4; i<8; i++) cmd[i] -= val; }
        if (has_key('s')) { for(int i=4; i<8; i++) cmd[i] += val; }
        if (has_key('a')) { cmd[4] += val; cmd[6] += val; cmd[5] -= val; cmd[7] -= val; }
        if (has_key('d')) { cmd[4] -= val; cmd[6] -= val; cmd[5] += val; cmd[7] += val; }
        return cmd;
    }
